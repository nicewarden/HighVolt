...
const STORAGE_KEY = 'highvolt-reference-library';

// Offline stand-in for the main app's server-backed Reference Library: same
// fields, persisted in the device's own localStorage instead of the
// SQLite-backed /api/reference routes (there's no server in this build).
function loadAll() {
  try {
...
export default function LibraryTab() {
  ...
  const submit = (e) => {
    ...
    const next = [{ ...form, id: Date.now() }, ...items];
    setItems(next);
    saveAll(next);
    setForm(EMPTY);
    setShowForm(false);
  ...
  const filtered = q
    ? items.filter(it => `${it.title} ${it.content} ${it.tags}`.toLowerCase().includes(q))
    : items;
  ...
  return (
    <div>
      ...
      <div className="form-row" style={{ maxWidth: 400 }}>
        <label>Search</label>
        <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search title, content, tags..." />
      </div>
      ...
      <div className="grid grid-3">
        ...
        {filtered.length === 0 && <p className="empty-state">No reference items yet.</p>}
      </div>
    </div>
  );
}
